...
export const usePreferencesStore = create((set, get) => ({
  ...
  async loadPreferences() {
    ...
    // Push the saved model server URL/model down to the main process's
    // llamaEngine (src/main-native/llamaEngine.js) on every load - it
    // only lives in memory over there, so each fresh app launch needs
    // this pushed once preferences are available (the user_preferences
    // table itself is created by initDatabase() in the renderer, so the
    // main process can't just read it directly at boot - see the comment
    // in electron/main.js).
    if (result.data?.model_server_url) {
      import('../native-clients/llamaEngine').then(({ setServerConfig }) =>
        setServerConfig(result.data.model_server_url, result.data.model_server_model).catch(() => {})
      );
    }
  ...
  /**
   * Saves the person's model server URL + chosen model id (Settings >
   * Model Server) and pushes the same values down to the main process's
   * llamaEngine (src/main-native/llamaEngine.js) so the very next
   * sendMessage() call uses them immediately, without waiting for a
   * restart or a stale in-memory copy. Replaces the old
   * grantModelFolderAccess flow entirely - there's no folder grant or
   * file copy anymore, just a URL and (optionally) a model id.
   */
  async setModelServerConfig(url, modelName) {
    const result = await updatePreferences({ model_server_url: url, model_server_model: modelName });
    if (result.success) {
      const { setServerConfig } = await import('../native-clients/llamaEngine');
      await setServerConfig(url, modelName);
      await get().loadPreferences();
    }
    return result;
  },
