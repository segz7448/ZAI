...
const SENT_IMAGES_DIR = `${FileSystem.documentDirectory}zai-sent-images/`;
...
export const useChatStore = create((set, get) => ({
  ...
  async editMessage(messageId, newContent) {
    ...
    if (result.success) {
      ...
      await get().loadConversationList();
      syncNow().catch((err) => console.error('[ChatStore] background sync failed:', err));
      if (prefs.memory_enabled !== false) {
  ...
  async sendMessage(userText, attachment = null) {
    ...
    if (attachment) {
      ...
      if (result.isImage) {
        ...
        // is not sent to any model - ZAI is fully local/text-only now.
    ...
    // everything ZAI remembers about the person from past conversations
