 ...
 *
 * The model itself now runs on a separate model server the person starts
 * themselves (e.g. llama.cpp's llama-server) - see Settings > Model
 * Server. Whatever they picked there (model_server_model preference,
 * from the presets in src/config/localModels.js, or a custom id) is what
 * gets sent with every request; there's no per-message routing between
 * different local models anymore, since that's now the person's own
 * choice of what to run on their server, not something ZAI picks. There
 * is also no fallback chain: a failure here (server not configured,
 * unreachable, or erroring) is a real failure and is surfaced to the
 * person as such rather than silently retried elsewhere.
 *
 ...
 * ZAI is fully local/text-only now.
 *
...
import { classifyTask, LOCAL_MODELS, MODEL_KEYS } from '../config/localModels';
import * as llamaEngine from '../native-clients/llamaEngine';
import { runGithubTask } from '../services/toolOrchestrator';

/**
 * Resolves the model label/id the person actually configured in Settings
 * > Model Server, falling back to a generic label if they haven't picked
 * a preset (model_server_model unset means "let the server decide").
 */
function resolveConfiguredModel(preferences) {
  const configuredId = preferences?.model_server_model || null;
  const preset = Object.values(LOCAL_MODELS).find((m) => m.serverModelId === configuredId);
  return {
    modelKey: preset?.key || MODEL_KEYS.CUSTOM,
    modelId: preset?.label || configuredId || 'Model server (default)',
    serverModelId: configuredId,
  };
}
...
export async function sendMessageOrchestrated({
  ...
  try {
    ...
    const { modelKey: configuredModelKey, modelId: configuredModelId, serverModelId } = resolveConfiguredModel(preferencesResult.data);
    ...
    // ========================================================================
    // NORMAL CHAT COMPLETION - whatever model the person's server is
    // configured to run (Settings > Model Server), no fallback: a failure
    // here (server not configured/unreachable/erroring) is real and gets
    // surfaced as such.
    // ========================================================================
    const result = await llamaEngine.sendMessage(history, serverModelId, { maxTokens: 1024, temperature: 0.7 });
