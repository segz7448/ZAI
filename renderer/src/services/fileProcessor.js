/**
 * ZAI - File Processing Orchestrator
 *
  ...
  try {
    if (isPptx(name, mimeType)) {
      return {
        ...
        text: null,
        truncated: false,
        error: 'PowerPoint (.pptx) reading isn\'t supported yet in ZAI - it needs dedicated slide-parsing that hasn\'t been built. PDF, Word, ZIP, CSV, and text/code files all work.',
      };
    }
    ...
    switch (category) {
      ...
      default:
        return {
          ...
          text: null,
          truncated: false,
          error: `ZAI doesn't know how to read "${name}" yet. Supported: PDF, Word (.docx), ZIP, CSV, and text/code files.`,
        };
    }
  } catch (err) {
