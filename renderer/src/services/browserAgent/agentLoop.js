/**
 * ZAI - Browser Agent Loop (Stage 2)
 *
...
const SYSTEM_PROMPT = `You are ZAI's on-device browser agent. You control a real browser on the
...
{"action": "click", "zaiId": "z3"}
{"action": "fill", "zaiId": "z5", "text": "..."}
{"action": "selectOption", "zaiId": "z2", "value": "..."}
{"action": "setChecked", "zaiId": "z7", "checked": true}
{"action": "submitForm", "zaiId": "z5"}
{"action": "scrollTo", "zaiId": "z9"}
...
export class AgentSession {
   ...
   *   Supabase snapshot recording is wanted for this session (optional -
  ...
  async _executeAction(action) {
    ...
    switch (action.action) {
      ...
      case 'click':
        await view.click(action.zaiId);
        await new Promise((r) => setTimeout(r, 800)); // let click-triggered nav/DOM changes settle
        return view.extractInteractiveElements();
      case 'fill':
        await view.fill(action.zaiId, action.text);
        return view.extractInteractiveElements();
      case 'selectOption':
        await view.selectOption(action.zaiId, action.value);
        return view.extractInteractiveElements();
      case 'setChecked':
        await view.setChecked(action.zaiId, action.checked);
        return view.extractInteractiveElements();
      case 'submitForm':
        await view.submitForm(action.zaiId);
        await new Promise((r) => setTimeout(r, 1500));
        return view.extractInteractiveElements();
      case 'scrollTo':
        await view.scrollTo({ zaiId: action.zaiId });
        return view.extractInteractiveElements();
  ...
  async _runTaskInner(taskText, onStep) {
    ...
    for (let step = 0; step < MAX_STEPS_PER_TASK; step++) {
      ...
      // Step-by-step recording: one snapshot per executed action, uploaded
      // to Supabase Storage under this session's id. Skipped for
