/**
 * ZAI - Local SQLite Database Layer
 *
 * Design principles:
 ...
 * - This is the offline-first source of truth. Supabase sync happens on top of this,
 *   never instead of it.
 */

import * as SQLite from '../native-clients/sqliteShim';
import * as SecureStore from '../native-clients/secureStoreShim';

const DB_NAME = 'zai.db';
let dbInstance = null;
...
export async function initDatabase() {
  try {
    ...
    await db.execAsync(`
      ...
      -- into every NEW conversation so ZAI doesn't start from zero each
      ...
      --
      -- source_conversation_id is kept for traceability ("where did ZAI
      ...
      -- person deliberately asked ZAI to forget.
    ...
    // src/services/llama/modelImportTool.js). ZAI's chat/coding/reasoning
    ...
    // 'zai-files' Supabase bucket in the background (see
    ...
    // Migration: model_server_url / model_server_model added when local
    // inference moved from an in-process node-llama-cpp context
    // (src/main-native/llamaEngine.js) to a separate, person-run model
    // server (llama.cpp's llama-server or compatible OpenAI-style HTTP
    // API). Bundling a native llama.cpp binding directly in Electron's
    // main process meant one bad native-module load (wrong ABI, missing
    // GPU backend, etc.) could throw before the window ever painted,
    // producing exactly a blank window with only the native menu bar.
    // Running the model out-of-process removes that whole failure class:
    // the app is just an HTTP client now. model_server_url is the
    // server's base URL (e.g. http://localhost:8080), and
    // model_server_model is an optional model identifier to pass through
    // to servers that host more than one model at once (e.g. llama-swap).
    try {
      await db.execAsync(`ALTER TABLE user_preferences ADD COLUMN model_server_url TEXT;`);
    } catch (migrationErr) {
      // Expected on any install that already has this column - not an error.
    }
    try {
      await db.execAsync(`ALTER TABLE user_preferences ADD COLUMN model_server_model TEXT;`);
    } catch (migrationErr) {
      // Expected on any install that already has this column - not an error.
    }
...
export async function addMessage(message) {
  try {
    ...
    const {
      ...
      local_image_path = null, supabase_image_path = null,
    } = message;
    const now = Date.now();

    await db.runAsync(
      `INSERT INTO messages
        (id, conversation_id, role, content, provider, model, model_family, token_count, created_at, sync_status, is_error, local_image_path, supabase_image_path)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)`,
      [id, conversation_id, role, content, provider, model, model_family, token_count, now, is_error ? 1 : 0, local_image_path, supabase_image_path]
    );
...
/**
 * Records the Supabase storage_path for a message's generated image once
 * the background upload (see src/storage/fileStorage.js uploadFile()) has
 * completed. Safe to call even if the message was already deleted/edited
 * away - just a no-op UPDATE affecting zero rows.
 */
export async function setMessageSupabaseImagePath(id, supabaseImagePath) {
  try {
    const db = await getDb();
    if (!db) return { success: false, error: 'DB_OPEN_FAILED' };
    await db.runAsync(`UPDATE messages SET supabase_image_path = ? WHERE id = ?`, [supabaseImagePath, id]);
    return { success: true, error: null };
  } catch (err) {
    console.error('[DB] setMessageSupabaseImagePath failed:', err);
    return { success: false, error: err?.message || 'UNKNOWN_ERROR' };
  }
}
...
export async function markMessageSynced(id) {
  try {
    const db = await getDb();
    if (!db) return { success: false, error: 'DB_OPEN_FAILED' };
    await db.runAsync(`UPDATE messages SET sync_status = 'synced' WHERE id = ?`, [id]);
    return { success: true, error: null };
  } catch (err) {
    console.error('[DB] markMessageSynced failed:', err);
    return { success: false, error: err?.message || 'UNKNOWN_ERROR' };
  }
}

export async function getPendingSyncMessages(limit = 100) {
  try {
    const db = await getDb();
    if (!db) return { success: false, error: 'DB_OPEN_FAILED', data: [] };
    const rows = await db.getAllAsync(
      `SELECT * FROM messages WHERE sync_status = 'pending' ORDER BY created_at ASC LIMIT ?`,
      [limit]
    );
    return { success: true, data: rows || [], error: null };
  } catch (err) {
    console.error('[DB] getPendingSyncMessages failed:', err);
    return { success: false, error: err?.message || 'UNKNOWN_ERROR', data: [] };
  }
}
 ...
 * @param {string} [detail] - short human-readable label, e.g. "Pushed to segz7448/ZAI"
...
export async function updatePreferences(patch) {
  try {
    ...
    const fields = [];
    const values = [];
    for (const key of ['theme_preference', 'tts_voice_identifier', 'tts_speech_rate', 'tts_voice_preset', 'voice_mode_activation', 'browser_router_url', 'browser_access_enabled', 'github_username', 'filesystem_saf_uri', 'model_folder_saf_uri', 'memory_enabled', 'model_server_url', 'model_server_model']) {
...
// removed entirely - ZAI has no vision, image generation, or speech
